...
generate the graph of Manhattan using networkx.
    input:  edges.csv
            nodes.csv
            time-on-week.csv
    output: ./pickle-files-gitignore/NYC_NET.pickle
"""
...
def load_Manhattan_graph():
    ...
    for i, edge in rng:
        ...
        # print(u, v, 'mean_travel_time', mean_travel_time, 'std', std, 'travel_dist', travel_dist, 'var', variance)
    ...
    if not os.path.exists(path):
        os.mkdir(path)
    with open(path + 'NYC_NET_WEEK.pickle', 'wb') as f:
        pickle.dump(G, f)
